...
MarkdownImageRenderer   读取 MDAST image 节点，解释 ![]() 的语义
...
Markdown 适配层只处理 MDAST 图片节点。

```ts
function renderMarkdownImage(node: Image, options: Readonly<RichContentOptions>) {
  const safeSrc = sanitizeImageUrl(node.url, options);
  if (!safeSrc) return node.alt ?? node.url;
  ...
  return h(UniverseImage, {
    ...
    defaultVisible: true,
    allowToolbox: false,
    showCaption: Boolean(node.title),
  });
}
```
