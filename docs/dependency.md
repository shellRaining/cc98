...
`apps/website` 使用 `@milkdown/kit` 提供 Markdown 编辑器，按需组合 CommonMark、GFM、history、listener 和 upload。阅读侧使用 `remark-parse`、`remark-gfm` 与 `unified` 生成 MDAST。项目不依赖 Crepe 完整入口、`md-editor-v3` 或 `markdown-it`。升级 Milkdown 与 remark 时，需要同时验证 Markdown 往返、MDAST renderer、写作路由 chunk 和 URL 安全测试。
