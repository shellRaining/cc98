...
colors:
  ...
  text-caption: "#6b7178"
  link: "#0b5394"
  inverse-surface: "#1a1d21"
  inverse-text: "#ffffff"
  shadow: "rgb(0 0 0 / 0.16)"
  shadow-strong: "rgb(0 0 0 / 0.35)"
  success: "#2f9e6d"
  warning: "#b8770a"
  error: "#e5484d"
  on-primary: "#ffffff"
...
标识蓝取老论坛沿用已久的板块色 `rgb(51,153,255)`（`brand`，即 #3399ff，见 `Forum/Styles/Site.scss` 的 `$color-board`）。夏季皮肤主色 `primary` 使用原站的 #5198d8，标题、链接和图标跟随皮肤主色；栏目装饰和实色控件分别使用 `primary-soft` 与 `primary-fill`。正文链接沿用原站的深蓝 #0b5394，访问前后保持同色，悬停时不加下划线。页面以 #e6e7ec 为底，白色面板承载内容。
...
- **Secondary fill（#6b7178）**：不随皮肤变化的中性实色表面，用于版面身份区等需要稳定承载浅色文字的次级区域。皮肤次色只做文字、图标和装饰，不直接作为浅色文字背景。
...
- **反色表面**：图片工具条、代码块等固定深色区域使用 `inverse-surface` 和 `inverse-text`，不能在业务组件中直接写黑白色。它们与页面表面分开，暗色模式下仍保持足够层级。
- **阴影**：普通浮层使用 `shadow`，需要更强分离度时使用 `shadow-strong`。业务组件不直接写带透明度的黑色；彩色光晕和高光从对应语义色通过 `color-mix()` 派生。
...
改颜色前先查本节、`styles/global.css` 和 `uno.config.ts`，优先复用已有语义。模板中的颜色使用 `text-cc98-*`、`bg-cc98-*` 等 UnoCSS 语义类，组件样式使用 `var(--cc98-color-*)`；透明度、渐变和高光从语义色通过 `color-mix()` 派生。业务组件不能直接引用 `--cc98-blue`、`--cc98-paper` 等原始调色板变量，也不能根据皮肤名选择颜色。

全局 token 只在现有语义无法表达用途时新增，并且需要满足至少一个条件：有两个独立消费者、需要被亮暗模式或皮肤单独覆盖，或者会成为公共组件的稳定视觉接口。只服务一个页面的装饰色优先从现有语义色派生，不进入全局调色板。

新增 token 的同一个改动必须包含用途说明、默认亮暗结果、需要的皮肤覆盖、至少一个真实消费者，以及模板需要使用时的 UnoCSS 映射。亮暗模式可以复用同一个值，但要确认视觉结果。删除最后一个消费者时同步删除语义变量、原始变量、UnoCSS 映射和本节说明，不保留备用 token。

颜色改动完成后运行 `vp run website#check:colors`。检查会阻止颜色字面量、业务组件直接读取原始调色板，以及引用不存在的语义变量；提交前仍需按改动范围走亮暗模式和相关皮肤的浏览器回归。
...
桌面端用固定最大宽度的居中栏，内容区宽 `content-width`（1140px），沿用老论坛的版心（`Forum` 的 `min-width: 1140px` / `71.25rem`），两侧留白。当前代码用的 `max-w-6xl`（1152px）迁移时统一到 1140px。顶栏已适配窄屏，其他页面仍需逐步适配。

间距走 4px 基准的比例尺（`xs` 4 到 `3xl` 48），列表项、卡片内边距、区块间隔都从这套尺度取值。首页沿用 1140px 内容区，热门话题在宽屏分两列，其他栏目按可用宽度自动排列。

页面骨架是顶栏、居中内容、页脚三段。首页顶部有 192px 横幅承载皮肤背景图，48px 顶栏叠在横幅上；内页只保留 48px 顶栏。顶栏固定在视口顶部，窄屏时增高并分行展示导航与搜索。浅色顶栏跟随皮肤顶栏色，暗色顶栏使用带皮肤色调的深色表面，不使用固定蓝。
...
- 需要浅色文字的中性区域使用 `secondary-fill`，不要把随皮肤变化的 `secondary` 直接当作填充色。
...
- 黑白反色区域、阴影和半透明高光也使用语义 token 或由语义色派生，不把 `#fff`、`#000`、固定 `rgb()` 当作例外散落在组件里。
