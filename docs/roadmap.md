...
阶段 0 至阶段 3 已完成；公共 API 基础设施和核心阅读闭环已经建立。当前准备进入阶段 4「发现与列表」。

`BoardView` 已提供版面信息、置顶与普通主题列表以及无总数分页；`TopicView` 已提供主题信息、完整楼层、分页和楼层锚点。需要登录的阅读接口会保存来源页，登录后返回原位置。阶段 4 在这些共享列表、分页和页面状态组件上继续扩展发现类页面。
