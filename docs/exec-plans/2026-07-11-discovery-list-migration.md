...
路线图仍显示阶段 3「核心阅读闭环」未完成，当前代码也与此一致：`BoardView` 还是占位页，`TopicView` 缺少主题信息、完整楼层 UI 和共享分页。因此第四阶段不能直接开始页面实现，应先完成阶段 3，或者至少先交付第四阶段依赖的共享列表与分页基础设施。
...
- `topicSchema`、`boardSchema`、`meUserSchema` 等基础数据模型。
- `/topic/:topicId/:page?`、`/list/:boardId/:type?/:page?`、`/boardlist` 路由骨架。
- `ContentRenderer` 及 UBB、Markdown 安全渲染。

第四阶段开始前仍缺少：

- 可在版面和发现页面复用的 `TopicListItem`。
- URL 驱动的共享分页组件和分页参数解析。
- 统一的页面加载、空数据、无权限、未登录、404 和服务端错误展示。
- 主题列表响应中的版面名、头像、标签、媒体摘要等字段是否需要独立 schema。
- 公开用户 schema、用户近期主题 schema 和对应 query。
...
- [ ] 完成阶段 3 执行计划并实现版面主题列表、主题详情信息、楼层 UI 和共享分页。
- [ ] 从阶段 3 提取 `TopicListItem`、`Pagination`、页面状态组件和路由参数解析。
- [ ] 更新 `docs/roadmap.md`，只有阶段 3 达到完成定义后才把当前阶段切换为阶段 4。

### 1. 接口探测与契约固定

- [ ] 使用匿名和测试账号分别探测第四阶段接口，保存脱敏后的响应形状和状态码结论。
- [ ] 核对 OpenAPI 与旧前端接口路径，确认热门周期、推荐包装结构、用户字段和搜索限频语义。
- [ ] 确认新帖、搜索、用户近期主题的分页终止规则，避免继续沿用“多取一条”的隐式约定。
- [ ] 确认哪些入口仍在现网使用，删去已经废弃的媒体新帖或历史热门入口。

### 2. 扩展 API 层

- [ ] 扩展主题、搜索、版面搜索、公开用户和推荐响应 schema。
- [ ] 增加热门、新帖、推荐、搜索、版面搜索和用户查询。
- [ ] 为分页查询和无限查询建立稳定 query key。
- [ ] 补充状态码到页面错误类型的映射，不在每个页面重复判断原始异常。

### 3. 完善共享列表基础设施

- [ ] 扩展阶段 3 的主题列表项，覆盖版面、用户、统计和可选摘要。
- [ ] 支持分页和加载更多两种列表容器。
- [ ] 实现加载、空数据、无权限、限频、404 和通用错误状态。
- [ ] 为用户、版面和主题链接统一生成规则，处理匿名或已注销用户。

### 4. 迁移热门与新帖

- [ ] 实现 7 日、30 日和历史热门页面，复用同一页面骨架和不同 query 参数。
- [ ] 实现新帖加载更多，恢复滚动位置和已加载数据由 Vue Query 管理。
- [ ] 如保留媒体新帖模式，把模式写入 URL 并与普通新帖共享页面。
- [ ] 把首页热门占位查询改为正式 query，并链接到完整热门页面。

### 5. 迁移推荐

- [ ] 确认推荐响应结构和推荐理由字段。
- [ ] 实现“换一批”和当前会话内“上一批”。
- [ ] 处理固定 query key 导致随机结果不刷新的缓存问题。
- [ ] 登录失效时保留来源页和当前批次交互状态。

### 6. 迁移搜索

- [ ] 实现搜索表单、关键词和版面筛选的 URL 同步。
- [ ] 实现全站主题搜索和版面内主题搜索。
- [ ] 实现版面搜索，并兼容旧 `/searchBoard` 链接。
- [ ] 处理空关键词、连续搜索、限频、无结果和输入法提交。

### 7. 迁移公开用户主页

- [ ] 实现按 ID 和用户名查询用户，成功后规范化到 ID 路由。
- [ ] 展示公开资料和近期主题，缺失或隐私字段不做猜测性占位。
- [ ] 近期主题使用加载更多，切换用户时清理页面级历史状态。
- [ ] 管理入口和个人中心入口只在已有权限与路由可用时展示。

### 8. 导航与文档收尾

- [ ] 在头部、首页或版面入口补充发现页面导航，避免只有手输 URL 才能访问。
- [ ] 更新 `docs/frontend.md` 中新增的 API、组件和 composable 边界。
- [ ] 阶段完成后更新 `docs/roadmap.md` 状态和相关执行计划链接。
- [ ] 如形成长期通用的分页或错误模型，再评估是否需要 ADR。
...
- 先完成阶段 3 前置任务，再开始阶段 4 页面实现。

## 待确认事项

1. 第四阶段是否保留旧项目的“媒体新帖”和“历史热门”入口。
2. 搜索、推荐、新帖和公开用户主页目前是否允许匿名访问，实施前需要测试账号和可访问的 API 环境确认。
3. 推荐页是否要保留“上一批”，还是只做“换一批”。
4. 用户主页的范围是否只包含公开资料与近期主题，用户管理应继续排除在本阶段之外。
...
- 2026-07-11：随机推荐使用显式刷新机制和会话内有限历史，不迁移 localStorage 缓存。
