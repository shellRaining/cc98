...
- 更新 README、架构、依赖说明和路线图，写清事实源与生成边界。
...
- [ ] 扫描当前项目 query、旧项目 API 调用和 `@cc98/api` 声明，生成初始 endpoint 清单。
- [ ] 为每个 endpoint 标记阶段、认证、风险、来源和验证状态。
- [ ] 合并路径参数写法不同但语义相同的调用，保留冲突记录。
- [ ] 识别旧项目引用但当前后端可能已经废弃的接口。

### 2. 工具链试验

- [ ] 用 3 到 5 个阶段 3 接口编写最小 OpenAPI 3.1 样例。
- [ ] 试验多文件 lint、bundle、TypeScript 类型生成和静态文档。
- [ ] 检查生成 diff 稳定性、Node 22 兼容性和许可证。
- [ ] 根据结果确定依赖，并更新 `docs/dependency.md`。

### 3. 创建 `packages/api`

- [ ] 建立 package 元数据、目录和 Vite+ 检查配置。
- [ ] 建立按领域组织的 Zod schema 和 operation registry。
- [ ] 编写 server、tags、security scheme 和公共参数。
- [ ] 加入 `x-cc98-*` 验证扩展约定。
- [ ] 配置 OpenAPI 生成、接口目录生成和 package exports。

### 4. 验证全部只读接口

- [ ] 批量验证匿名只读接口，建立公开响应和错误响应 schema。
- [ ] 使用专用测试账号验证登录只读接口。
- [ ] 建立 `Board`、`Topic`、`Post`、`User`、`Message` 等领域 schema。
- [ ] 加入脱敏 fixture，记录样本数量和未覆盖分支。
- [ ] 记录线上行为与旧类型之间的差异。

### 5. 验证写入与高风险接口

- [ ] 准备专用测试账号、测试版面、测试主题和上传空间。
- [ ] 为可回滚写接口编写场景、清理逻辑和幂等保护。
- [ ] 按创建、编辑、互动、上传顺序验证，失败时停止后续依赖场景。
- [ ] 破坏性和管理接口逐项评估，只在具备隔离与恢复方案时实测。
- [ ] 未实测的高风险接口保留完整清单并标记 `legacy-observed`。

### 6. 接入当前网站

- [ ] 把网站已有 Zod schema 迁入公共包并保持 API 行为不变。
- [ ] 网站 query 直接导入公共 schema 和推导类型。
- [ ] 增加关键 fixture 同时通过 Zod 与生成 OpenAPI 的契约测试。
- [ ] 删除应用中已经迁移完成的重复 schema 和类型。
- [ ] 保持应用层 query key、错误映射和认证编排独立于规范包。
...
- [ ] 前端各阶段只消费和修正已有公共契约，不再在应用内新增平行 schema。
...
1. 这个包先留在当前 monorepo，还是立即建立独立公共仓库。建议先留在 monorepo，完成全接口清单和首批 schema 后再拆。
...
4. 是否能提供专用测试账号、测试版面和允许清理的测试主题。没有这些资源时只能安全验证只读接口。
5. 管理和破坏性接口是否有隔离环境或后端协助。没有时只能维护未验证契约，不能直接在生产环境探测。

## 进展与调整

- 2026-07-11：完成当前 monorepo、旧项目 API 类型和调用位置的第一轮盘点。
- 2026-07-11：确认当前仓库没有 OpenAPI 资产，`@cc98/ubb` 也尚未完成公共发布配置。
