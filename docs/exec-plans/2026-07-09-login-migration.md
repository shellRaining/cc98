...
- 2026-07-09：access token 存裸 token（`AuthData.accessToken` 为裸值），请求时由 `http.ts` 拼接 `Bearer ` 前缀。比老项目"存 Bearer 整串"更清晰，且登录/刷新两处行为一致（消解老项目登录不编码、刷新 encodeURIComponent 的历史不一致）。
- 2026-07-09：expiresAt 用 ms 时间戳（不做老项目截断 hack，更可读）。
- 2026-07-09：不做跨 tab storage 同步，留作后续增强。
- 2026-07-09：OAuth 端点用全局 fetch 而非 apiFetch（不同 host + form-urlencoded + 可测试性）。
- 2026-07-09：env 覆盖凭证通过 `configureOAuthClient` 运行时 setter 暴露（构建期走 `import.meta.env` 静态内联；测试用 setter，因 `vi.stubEnv` 在 vite-plus 静态内联下无法驱动 `import.meta.env`）。

## 完成情况

已完成。`vp run ready` 通过，42 项测试全绿，覆盖 token-store / oauth / ensureValidAccessToken 懒刷新编排。
