...
> 状态：已完成。设计 token、主题状态、换肤链路和首批基础组件已经落地；其余基础组件按实际复用需求补充，不作为未完成计划。

## 背景

阶段 0 至阶段 7 已完成，功能页面形成完整闭环，但视觉一直按路线图约定往后放。计划启动时，网站只有一份最小的明暗 CSS 变量（`styles/global.css` 七个颜色），UnoCSS 里手写了 `cc98-primary` 等少量 token 和三个 shortcut，主题状态只有 `mode + season` 两个维度，season 还没有任何实际皮肤对应。
...
- [x] 建立按需扩展的 `components/ui`，落地 Button、Dialog、Badge；Input、Textarea、Card、Tabs 仅在出现稳定复用需求时新增。
- [x] 迁移 Header、DefaultLayout、PageState、TopicList、PostItem 到语义 token 和基础组件。
- [x] 实现默认亮、默认暗和节日皮肤的 CSS 变量覆盖，并在全站高保真计划中完成浏览器回归。
- [x] 按首发 `solid` 风格迁移其余页面，再逐步补齐老论坛皮肤和 `elegant`、`fluent` 方向。
- [x] 完成无障碍对比度检查、修正和浏览器回归。
...
- 2026-07-18：原论坛提交 `1ab3b51a7177` 将 5/6 号秋季主题改为亮暗配对。注册表随之归约为 20 个 skin、10 对亮暗皮肤。
...
- 2026-07-16：建立 `components/ui` 基础组件层。`Button`（variant primary/ghost/danger、size md/sm、loading 自动 disabled、block、asChild 支持 RouterLink），迁移全部 15 处 `cc98-btn` 按钮消费方，loading 态文字切换不再手写。`Dialog` 统一封装 Reka UI Dialog 与 AlertDialog 两族原语（alert prop 切换），吸收 Portal/Overlay/Content/Title/Description/Close 的固定模板，提供 trigger/default/footer 插槽和 pending/cancelLabel/confirmLabel 快捷 prop；重构 ConfirmDialog（71→28 行）、PostRatingDialog、TopicFavoriteAction，每个消减约 24 行 import + 18 行模板样板。Card 和 Tabs 按需建立，现有 Card 场景使用样式 shortcut，路由驱动导航不使用 Reka Tabs 原语。
- 2026-07-16：新增 Badge 组件（count 截断 99+、dot 圆点、variant accent/primary），迁移 AppHeader 和 MessagesLayout 两处计数徽标及 NotificationListView/PrivateMessagesView 两处未读圆点。Button 新增 text 变体（纯文字按钮，替代 cc98-link），迁移全站剩余 20 处 cc98-link 按钮和 5 处 border 次操作按钮到 UiButton，全站不再有手写 cc98-btn/cc98-link 的 button 标签。
- 2026-07-16：落地皮肤 CSS 和无障碍对比度修正。global.css 补全 radius/spacing/content-width 尺度 token（DESIGN.md 已定义但代码里缺失）。实现春节皮肤（金舞迎春）的 `[data-skin="spring-festival"]` 变量覆盖：亮色珊瑚红主色 #fb8071 + banner 背景图，暗色加深版 #c53d37 + 深底色面板，背景图拷贝自老论坛 dist 到 `public/skins/spring-festival/`。WCAG AA 对比度检查覆盖 21 组前景/背景组合：修正深色 accent 从 #ff6b6f 加深到 #e04248（白字 2.77:1→5.05:1），修正春节亮色 primary 从 #fb8071 加深到 #d4452f（白字 2.49:1→6.27:1），两者均从低于 3:1 提升到远超 UI 元素标准线。删除遗留的 `button.cc98-link` 元素选择器（全站 button 已迁移到 UiButton 组件，无消费方）。
...
- 标识色 `brand`（#3399ff，取自老论坛板块色 `$color-board`）做 Logo 和装饰。高保真 `solid` 风格的 `primary` 跟随皮肤主色，夏季为 #5198d8；正文链接使用旧站的 #0b5394，按钮在需要时使用单独的高对比操作色。
- Acrylic 只作为 `fluent` 风格的可选表现，必须有不透明回退，且不铺在正文上。
- 皮肤编号沿用老论坛服务端约定，本地维护编号到 `skin` 名的类型安全映射做互转，不改后端。
- 本计划只覆盖设计系统和换肤地基。首页信息架构、页面壳、版面与主题高保真布局、用户中心和边缘功能已按 `docs/exec-plans/completed/2026-07-16-full-fidelity-migration.md` 完成。
