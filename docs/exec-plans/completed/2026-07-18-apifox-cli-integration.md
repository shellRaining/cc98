...
> 状态：已终止。公开 OpenAPI 站点和 Apifox URL 数据源已经覆盖同步需求，仓库不再维护 CLI 同步链路。

## 调研结论

Apifox 的 URL 数据源支持目标模块、目标分支、冲突策略、目录同步、Security Scheme、Server 和删除源中不存在的资源。主 API 与 OpenID 可以分别绑定两份托管规范，保留各自的服务地址和认证语义。

CLI 方案需要在一次性导入后重新完成模块归类、鉴权修正、模型映射和临时资源清理，回读验证也只能覆盖部分结构，无法替代 Apifox 原生数据源配置。仓库已移除 `apifox-cli` 依赖，未完成的实现与云端写入验证不再属于交付物。以下内容保留为调研记录。

审查分支的 22 项专项测试已经通过，但当前用户态 CLI 未登录，因此没有读取或修改 Apifox 线上项目。该结果只证明本地编排逻辑可运行，不改变上述取舍。
...
- [ ] 完成两次幂等同步与 GUI 人工检查。
- [ ] 更新包内 README 并通过质量门禁。
- [x] 2026-07-25：确认改用托管 OpenAPI URL 数据源，终止 CLI 同步方案并移除固定依赖。
...
- 2026-07-25：主 API 与 OpenID 分别绑定托管规范，Apifox 的模块映射、冲突处理和资源清理由原生数据源配置负责。
