...
> 状态：已完成。发主题、回帖、编辑、上传、草稿恢复和主要楼层互动已经通过自动测试与浏览器回归。
...
- [x] 使用限定资源重新验证阶段 6 所需接口，记录响应类型、状态码、错误码和权限边界。
- [x] 按实测结果修正 operation registry、schema、fixture 和探测记录。
- [x] 实现 multipart 请求、上传、草稿、错误映射和共享 Markdown 编辑器。
- [x] 实现发主题入口、页面和 mutation。
- [x] 实现主题页回帖、引用和提交后定位。
- [x] 实现历史帖子读取、UBB 转换、编辑和返回楼层。
- [x] 实现点赞、评分、主题收藏和投票。
- [x] 补充自动测试，执行增量检查和 `vp run ready`。
- [x] 完成真实浏览器回归，记录截图、录屏和未解决差异。
- [x] 更新路线图、前端规范及必要的安全文档。
...
- [x] 楼层互动。
- [x] 回归与文档收尾。
- [x] 2026-07-18：高保真迁移和最终浏览器矩阵再次覆盖写作页面，阶段 6 完成状态与当前实现一致。
