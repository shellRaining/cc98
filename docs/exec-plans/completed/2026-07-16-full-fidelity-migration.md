...
| 签到             | 月历、补签、统计                                                                                   | 说明、留言签到、连续天数、财富奖励、补签规则和月历均已恢复                  | 已完成，签到与补签写入已通过真实接口验证   |
...
- 2026-07-20：核对真实接口后确认 `/post/topic/specific-user` 按楼层追踪关系筛选，不能支持“只看此人”。主题页移除该入口和 `userId` 查询状态，只保留使用 `postId` 的楼层追踪；赞踩状态改为同时更新普通楼层与热门回复缓存，并在写入成功后用独立状态接口校正。
...
- 2026-07-31：真实接口验证补齐签到与补签写入状态。签到成功响应确认为本次获得的财富值，公共契约新增 `SigninReward`，前端按该契约解析响应；两项写入均标记为 `verified-write`。
