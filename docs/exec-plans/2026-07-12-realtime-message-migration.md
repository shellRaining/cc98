...
- [ ] 使用限定账号和无副作用场景验证私信发送、三类全部已读和签到请求，记录请求体、响应和错误码。
- [ ] 补充非空私信会话 fixture，修正消息、通知、关系和签到 schema，重新生成 API 产物。
- [ ] 建立消息与签到 query key、queries、mutations 和缓存清理边界。
- [ ] 实现受限消息布局、通知列表、批量信息补充和分类全部已读。
- [ ] 实现私信联系人、会话路由、历史加载、发送和失败恢复。
- [ ] 实现头部未读入口和 SignalR 生命周期服务，再补跨标签失效通知。
- [ ] 在公开用户页和版面页接入关注与取消关注。
- [ ] 实现签到状态、今日签到和月度日历；补签只做到经过权限保护的产品入口，不执行无许可成功测试。
- [ ] 根据实测结果决定是否补充 Markdown @ 通知发送和私信图片上传。
- [ ] 补充自动测试，执行增量检查和 `vp run ready`。
- [ ] 使用真实浏览器验证通知、私信、关注、签到和实时刷新，保存截图、录屏与临时报告。
- [ ] 更新路线图、前端规范、API 探测记录及必要的安全文档。

## 验证

### 自动测试

- schema 和 fixture 覆盖非空联系人、私信、三类通知、未读数量和签到记录。
...
- 当前私信 fixture 的会话为空，消息字段和返回顺序仍需从非空会话确认。
- 私信使用历史 UBB 语义，但是否允许 Markdown 链接、文件链接和图片仍未知。
- 通知全部已读和读取私信可能改变真实账户状态，实测前应确认测试账号与测试消息范围。
