...
> 状态：待实施。当前只完成计划和 OpenID 规范准备，尚未安装 CLI、添加包内命令或同步 Apifox。
...
Apifox 项目 ID 为 `6295240`。截至 2026-07-18，本机尚未安装 `apifox`，npm 上的 `apifox-cli` 最新版本为 `2.2.7`。
...
| 文件                                 | 用途                                                  |
| ------------------------------------ | ----------------------------------------------------- |
| `packages/api/package.json`          | 固定 CLI 依赖并增加三个显式命令                       |
| `pnpm-lock.yaml`                     | 锁定 `apifox-cli` 及其传递依赖                        |
| `packages/api/.apifox/settings.json` | 保存项目 ID、目标分支、受管模块和规范路径等非敏感映射 |
| `packages/api/scripts/apifox.mjs`    | 实现本地检查、同步编排和远端结构验证                  |
| `packages/api/README.md`             | 保存 API 子包的长期使用说明                           |

优先把 `.apifox/settings.json` 放在 `packages/api`，由包内脚本按自身路径读取。实施前仍需确认官方 CLI 是否强制从仓库根目录读取同名文件。如果 CLI 有固定位置要求，可以把文件移到根目录，但长期说明仍写在 `packages/api/README.md`。
...
1. `apifox import` 能否直接导入现有模块，并指定覆盖、合并或删除策略。
2. CLI 的默认分支是什么，如何明确指向主分支。
3. OpenAPI 已删除的接口在再次导入后会怎样处理。
4. 模块能否整体替换；若不能，如何只清理同步脚本管理的资源。
5. `.apifox/settings.json` 是否有官方固定路径或 schema。

### 阶段 2：固定依赖与非敏感配置

- [ ] 把验证通过的 `apifox-cli` 版本作为 `packages/api` 的精确 devDependency，首次计划基线为 `2.2.7`。
- [ ] 运行 `vp install` 更新锁文件，确认 Node 22 环境可执行 CLI。
- [ ] 新增包内 `.apifox/settings.json`，记录项目 ID `6295240`、主分支目标、两份规范路径和受管模块标识。
...
- [ ] 选择 CLI 已验证支持的幂等策略。优先使用模块范围内的替换导入；如果不支持，则删除并重建受管模块，或者逐项更新和清理。
- [ ] 清理范围只能覆盖配置中声明的受管模块，不能删除用户在 Apifox 中维护的其他接口、示例、测试场景或文档。
- [ ] 导入时显式指定项目、主分支、模块和格式，不能依赖当前目录之外的隐式状态。
- [ ] 保存命令输出中的资源 ID 只用于本次运行；需要长期稳定的非敏感模块 ID 时写入 settings。
