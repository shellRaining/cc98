...
| 主题阅读         | 主题信息区、用户侧栏、头像相框、等级资料、楼层、签名、奖励、赞踩、引用、只看、评分、管理、热门回复 | 标题区、楼层卡片、引用、点赞、评分、编辑、收藏、投票 | 保留现有写入逻辑，重做楼层骨架并补缺失入口     |
| 新帖             | 经典、卡片、仅媒体三种视图                                                                         | 单一列表                                             | 评估线上仍在使用的视图，至少恢复经典和媒体筛选 |
| 关注             | 关注版面、关注用户、收藏更新三页                                                                   | 数据散落在用户中心，没有 `/focus` 聚合页             | 恢复聚合入口，复用现有 query 和组件            |
| 精选             | 随机推荐和刷新历史                                                                                 | 随机推荐已可用                                       | 调整展示与刷新反馈，保留新实现                 |
| 搜索             | 顶栏内按主题、用户、版面、版内切换                                                                 | 主题和版面独立页面，没有用户搜索入口                 | 先恢复顶栏搜索模型，再补用户搜索               |
| 公开用户页       | 资料、动态、主题、回复、管理入口                                                                   | 资料、主题和回复已具备一部分                         | 补资料细节、活动流和有权限的管理入口           |
| 用户中心首页     | 头像、资料、最近活动、收藏摘要                                                                     | 统计卡片和导航                                       | 按原站信息层级重排，不复制旧组件结构           |
| 资料设置         | 头像、签名、个人资料、其他偏好                                                                     | 缺失                                                 | 新增设置页，复用 `/me`、头像上传和现有 schema  |
...
- 恢复 `/focus` 聚合页，包含关注版面、关注用户和收藏更新。
- 新帖页至少支持经典和仅媒体视图，卡片视图是否保留以线上使用情况决定。
- 精选页保留随机刷新历史和上一次结果。
...
1. `M2` 版面列表：恢复分区主管、展开收起、版面图标、今日与总帖数、右侧快速分区导航。
2. `M3` 版面页：恢复版面头区、标签筛选、精华、保留、版务记录和紧凑主题列表。
3. `M4` 主题阅读：恢复用户侧栏与楼层网格，并在现有写入逻辑上补齐签名、奖励、只看此人和管理入口。
