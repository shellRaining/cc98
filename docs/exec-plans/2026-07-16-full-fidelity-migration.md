...
| 主题设置         | 30 个旧编号、日夜自动切换、时间设置                                                                | 21 套皮肤、明暗模式、昼夜规则和真实预览均已恢复                             | 已完成，30 个旧编号可以双向转换            |
...
主题中心使用 `/usercenter/theme`，保留旧站 14rem × 6rem 的缩略图按钮和点击即保存行为。旧站 30 个编号继续归约为 21 个 `skin`，亮暗交给独立 `mode`；系统默认明确表示跟随站点默认，当前解析为夏季。21 套皮肤均使用旧站真实横幅缩略图，全部可以选择。昼夜设置支持浏览器明暗同步、固定时间切换和独立的亮色、深色模式，保存失败时回滚本地状态。
...
截至 2026-07-18，全皮肤切片已经完成。21 套皮肤恢复旧站主色、次色、顶栏色、页面底色、正文底色、横幅和卡片背景，30 个旧编号仍能双向转换。春节、冬日暖雪和新岁花朝使用独立的亮暗横幅，其余配对皮肤共用图片并切换颜色。审计同时修正了一个历史错位：原 `spring-festival` 目录存放的是 25/26 号金舞迎春资源，现已恢复为 12/13 号春节，金舞迎春单独放入 `golden-spring`。
...
- 2026-07-18：完成 M9 全皮肤迁移。旧站 30 个主题编号归约成 21 套皮肤，全部恢复主色、次色、顶栏色、亮暗底色、首页横幅、卡片横幅和主题中心预览。春节、冬日暖雪和新岁花朝保留独立暗色图片，其余亮暗配对皮肤共用图片。
