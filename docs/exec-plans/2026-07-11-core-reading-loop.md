...
- [ ] 使用匿名和登录状态探测版面信息、普通主题、置顶主题、主题信息和帖子接口。
- [ ] 记录真实响应形状、401/403/404 语义和分页终止条件。
- [ ] 确认主体版面是否允许匿名访问，受限版面返回 401、403 还是 `canEntry=false`。
- [ ] 确认 `board.topicCount` 能否可靠计算普通主题总页数。
- [ ] 确认置顶主题是否会重复出现在普通主题结果中。

### 2. 建立共享基础设施

- [ ] 增加路由 ID、页码和 `from` 换算工具。
- [ ] 增加安全的登录来源页存取与校验。
- [ ] 增加 API 错误归一化和共享页面状态组件。
- [ ] 实现 URL 驱动的 `Pagination`。

### 3. 固定 API 契约

- [ ] 根据探测结果修正 `boardSchema`、`topicSchema` 和 `postSchema`。
- [ ] 增加版面信息、置顶主题和主题信息 query。
- [ ] 修正 query key，确保版面、主题、页码和每页数量均参与缓存键。
- [ ] 为依赖参数设置查询启用条件，非法 ID 不发送请求。

### 4. 实现版面主题列表

- [ ] 实现 `TopicListItem` 和 `TopicList`。
- [ ] 实现版面标题、描述、置顶主题和普通主题。
- [ ] 实现普通主题分页和页码规范化。
- [ ] 处理匿名作者、已注销用户、空标题、缺失最后回复人和高亮样式。
- [ ] 把首页与版面列表中的链接统一到实际版面路由。

### 5. 完成主题阅读页

- [ ] 请求主题信息和版面信息，展示标题、面包屑、统计和状态。
- [ ] 抽出 `PostItem`，将现有富内容渲染接入完整楼层 UI。
- [ ] 实现主题总页数、分页和超范围页码规范化。
- [ ] 实现楼层锚点与旧数字 hash 兼容。
- [ ] 处理删除帖、不可见帖、匿名用户和已注销用户。

### 6. 接入权限与返回链路

- [ ] 版面和主题页面区分需要登录、无权限、404 和服务端错误。
- [ ] 需要登录时保存当前完整路径并跳转登录。
- [ ] 登录成功返回来源页，非法或外部来源页回到首页。
- [ ] token 失效后清理登录态，并允许用户重新登录回到原阅读位置。

### 7. 测试与浏览器回归

- [ ] 为参数解析、分页窗口、来源页校验和错误映射补纯逻辑测试。
- [ ] 为版面列表和主题楼层补关键状态的组件测试。
- [ ] 用匿名和测试账号完成首页到版面、版面到主题、分页和登录返回回归。
- [ ] 验证 UBB、Markdown、图片、引用和媒体在楼层卡片中的布局。
- [ ] 更新路线图，把阶段 3 标记完成并链接本计划。
...
1. 版面首版是否必须提供“最佳”“精华”和标签筛选。建议阶段 3 只做普通主题和置顶主题，把其他筛选放入后续小阶段。
2. 楼层首版是否要求头像和签名。建议有稳定头像字段就展示头像，签名延后。
3. 旧数字 hash，例如 `#10`，是否必须兼容。建议兼容，避免现有站内和外部链接失效。
