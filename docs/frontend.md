...
Service Worker 只预缓存首页 App Shell，包括全局入口、首页路由依赖和少量界面图片。其他路由的懒加载 chunk、皮肤横幅和卡片预览图不进入预缓存。皮肤正式横幅按访问写入独立的 Cache Storage，使用内容哈希更新并限制条目数量；缓存条件同时检查 `200` 状态和 JPEG 内容类型，避免 SPA 回退响应被误存成图片。
