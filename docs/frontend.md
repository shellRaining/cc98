...
生产环境的 `/assets/*` 均带内容指纹，使用一年期 `immutable` HTTP 缓存；`index.html`、Service Worker 和注册脚本必须保持可重新验证。Vercel 的 SPA 回退只处理业务路由，静态资源、Service Worker、Manifest 和固定 PWA 文件必须保留真实的错误响应，不能用 `index.html` 代替。

Service Worker 只预缓存首页 App Shell，包括全局入口、首页路由依赖、Web App Manifest 和少量界面图片。构建插件从 Rollup 的网站入口和 `HomeView` 动态入口递归收集静态依赖、关联样式和首页直接引用的图片，拆包结果变化后不需要维护文件名通配规则。其他路由的懒加载 chunk、皮肤横幅和卡片预览图不进入预缓存。

访问其他路由后，其带内容指纹的脚本、样式和字体通过 `CacheFirst` 写入有条目上限的运行时缓存，离线或新部署切换后仍可使用已经访问过的页面代码。缓存写入和读取都检查 `200` 状态及资源类型，HTML 与错误响应不能作为脚本、样式或字体使用；校验规则变更时继续复用原缓存名，由有效网络响应替换不合规条目。正式横幅和皮肤雪球君按访问写入各自的 Cache Storage，使用内容哈希更新并限制条目数量。

应用外壳只保证页面代码和首页界面能启动，不缓存 API 响应、头像、帖子图片、附件或第三方媒体。离线状态下，已访问路由可以打开页面代码，但依赖服务端的数据继续进入现有错误状态。未访问过的路由不承诺首次离线打开。

网站通过 Web App Manifest 提供名称、启动地址、独立窗口显示模式和安装图标。Service Worker 注册代码随应用入口打包，不单独生成 `registerSW.js`。页面重新可见、恢复联网和持续打开期间会主动检查更新，检查需要合并并发请求并限制频率。发现新 Worker 进入等待状态后，页面显示更新提示，用户确认刷新时才切换版本。联网时，Vite 的预加载错误会显示中性的资源加载失败提示并触发更新检查；离线时保留离线提示和原有错误处理，不把网络失败误报为版本失效，也不自动刷新正在编辑内容的页面。

Cache Storage 负责请求与响应形式的静态资源。localStorage 不保存 Base64 图片，IndexedDB 留给草稿、待同步操作等结构化业务数据。扩展离线能力前先检查生成的预缓存清单和安装流量，不把 API 响应直接纳入 App Shell。
...
- `ubb/` 通过 `@cc98/ubb/cc98` 的 `cc98Registry` 解析 AST，并用包的泛型 renderer 登记 Vue 输出 handler（输出 `VNodeChild[]`），遍历由 renderer 统一完成。渲染状态（图片计数等）由 `UbbRenderer` 每次渲染创建，经 `Context` 透传给 handler。
...
- `packages/api`、`packages/ubb` 和 `packages/utils` 通过 workspace catalog 使用 TypeScript 7。Vite+ 0.3.1 已包含满足当前构建需求的 tsdown，构建配置仍根据运行平台显式解析 TypeScript 7 原生包中的 `tsc` 可执行文件，确保声明构建使用 workspace 选定的版本。声明构建只过滤 tsdown 固定的 TypeScript 7 实验性提示，其他 warning 保持可见。
