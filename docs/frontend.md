...
- `packages/api`、`packages/ubb` 和 `packages/utils` 通过 workspace catalog 使用 TypeScript 7。生成声明文件时，构建配置根据运行平台解析 TypeScript 7 原生包中的 `tsc` 可执行文件，绕过 Vite+ 0.2.4 对 `@typescript/native-preview` 的旧依赖。
