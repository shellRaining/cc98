...
生产环境的 `/assets/*` 均带内容指纹，使用一年期 `immutable` HTTP 缓存；`index.html`、Service Worker 和注册脚本必须保持可重新验证。修改缓存头时不能把无内容指纹的站点根文件一并设为长期缓存。
...
访问其他路由后，其带内容指纹的脚本、样式和字体写入有条目上限的运行时缓存，离线时可以再次启动这些页面的代码。正式横幅和皮肤雪球君按访问写入各自的 Cache Storage，使用内容哈希更新并限制条目数量；缓存条件同时检查 `200` 状态和图片内容类型，避免 SPA 回退响应被误存成图片。

应用外壳只保证页面代码和首页界面能启动，不缓存 API 响应、头像、帖子图片、附件或第三方媒体。离线状态下，已访问路由可以打开页面代码，但依赖服务端的数据继续进入现有错误状态。未访问过的路由不承诺首次离线打开。

网站通过 Web App Manifest 提供名称、启动地址、独立窗口显示模式和安装图标。Service Worker 注册代码随应用入口打包，不单独生成 `registerSW.js`。发现新 Worker 进入等待状态后，页面显示更新提示，用户确认刷新时才切换版本。
