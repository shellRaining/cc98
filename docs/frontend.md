...
访问其他路由后，其带内容指纹的脚本、样式和字体通过 `CacheFirst` 写入有条目上限的运行时缓存，离线或新部署切换后仍可使用已经访问过的页面代码。缓存写入同时检查 `200` 状态和资源类型，HTML 与错误响应不能存到脚本、样式或字体 URL。正式横幅和皮肤雪球君按访问写入各自的 Cache Storage，使用内容哈希更新并限制条目数量。

应用外壳只保证页面代码和首页界面能启动，不缓存 API 响应、头像、帖子图片、附件或第三方媒体。离线状态下，已访问路由可以打开页面代码，但依赖服务端的数据继续进入现有错误状态。未访问过的路由不承诺首次离线打开。

网站通过 Web App Manifest 提供名称、启动地址、独立窗口显示模式和安装图标。Service Worker 注册代码随应用入口打包，不单独生成 `registerSW.js`。页面重新可见、恢复联网和持续打开期间会主动检查更新，检查需要合并并发请求并限制频率。发现新 Worker 进入等待状态后，页面显示更新提示，用户确认刷新时才切换版本。懒加载资源因部署切换而失效时，Vite 的预加载错误也进入同一提示流程，不自动刷新正在编辑内容的页面。
