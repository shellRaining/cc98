...
Issue 和 PR 应让 reviewer 不运行项目也能看懂问题、改动和验证结果。涉及界面、交互、响应式、主题皮肤或富内容渲染时，正文必须附截图；多步骤交互、时序问题或动态效果使用录屏。附件按 `docs/quality.md` 的规则从 `.artifacts/browser/` 上传，不把临时验证产物提交到仓库。
