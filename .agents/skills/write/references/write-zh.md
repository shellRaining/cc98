...
- **图 / 表旁文字复述**：紧邻一张图或表的正文，把图 / 表已经列出的项再用文字列一遍，或用「这张图更直观」这类预判句替图说话。删掉复述和预判，留判断和上下文；画图过程只在它本身是叙事时才保留。例外：「图后必要说明段」不算复读，不要删（见「图后说明段落」例外）。
...
NO: File System State 开销只有 5% 至 15%，几乎总是值得的；Verifier Agent 能提升 25% 至 40% 精度，但成本增加 1.5 至 3 倍
OK: File System State 开销只有 5% 至 15%，几乎总是值得的。Verifier Agent 能提升 25% 至 40% 精度，但成本增加 1.5 至 3 倍
