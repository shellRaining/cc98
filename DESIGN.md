...
桌面端用固定最大宽度的居中栏，内容区宽 `content-width`（1140px），沿用老论坛的版心（`Forum` 的 `min-width: 1140px` / `71.25rem`），两侧留白。当前代码用的 `max-w-6xl`（1152px）迁移时统一到 1140px。

响应式布局使用两级收缩：1000px 以下把固定侧栏、宽网格和多栏工具区改为单列或紧凑布局；640px 以下进入手机布局，内容区左右各留 12px。手机端优先重排和换行，不隐藏主导航、搜索、状态、核心信息或管理操作。只有表格、代码块等必须保留结构的内容可以在组件内部横向滚动，页面本身不得产生横向滚动。主要触控目标高度不低于 40px。
