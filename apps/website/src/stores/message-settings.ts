...
type MessageSettingsStorage = Pick<Storage, "getItem" | "setItem">;
...
export function loadMessageSettings(
  storage: MessageSettingsStorage = localStorage,
): MessageSettings {
  try {
...
export function shouldJumpToLatestReply(storage: MessageSettingsStorage = localStorage): boolean {
  return loadMessageSettings(storage).post === "是";
}
